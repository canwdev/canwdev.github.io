...
- 插入数据线，长按电源键和音量减；黑屏时松开电源键和音量减，按住音量加，进入 Fastboot 模式。
- 也可以在插入数据线并且重启状态下按住音量加键进入 fastboot

### LG G8X 如何进入 recovery

- 插入数据线，长按电源键和音量减；黑屏时松开电源键和音量减，按住音量加，进入 Fastboot 后按音量键，直到屏幕左上方显示“Press_volume_key_to_select...”，再按电源键确认，进入 Recovery。
- adb reboot recovery
- https://forum.xda-developers.com/t/unofficial-twrp-recovery-twrp-3-3-1-0.4201783/
### TWRP 触摸屏不可用

- 外接 USB 鼠标进行操作。
- 提示：可以把刷机包放在 SD 卡中（`/external_sd`）刷机。
...
使用【MindowsWOA工具包】中的【Mindows一键切换】切回 Android 系统（记得将 Android 系统的 `boot.img` 放在工具目录），切回 Android 后，再尝试进入 recovery 模式。

### Android 切回 Windows

- 安装 [Mindows助手APP](https://www.123pan.com/s/8eP9-KDTGA)，按软件内教程操作（将设备的 `uefi.img` 放到指定位置）
- 进入 TWRP，切换到 ab 分区的另一个分区；例如当前在 a 分区，则切换到 b 分区后重启

### G8X 如何进入 Mindows 工具箱的大容量存储模式

1. 先进入 `fastboot`，参考上文教程
2. 使用 Mindows 工具箱 V8 的【`进入大容量模式`】功能
3. 选择【`3.刷入 (会覆盖当前Boot. ab分区设备将刷入当前槽位)`】；目前（2024.1.21）还不支持通过 `UEFI` 菜单进入，以及 `fastboot` 临时启动
4. 刷入成功后重启设备，此时会卡在开机 logo；连接电脑后会出现多个硬盘，说明已进入大容量模式
5. 此时可以备份 Windows 系统，或重刷 Windows
6. 完成上述操作后，重启仍会进入大容量模式，因为已经替换了 `boot`；此时需进入 `fastboot` 模式重新刷入 `UEFI`
7. 同时长按电源键、音量加、音量减，直到设备重启，再按住音量加进入 `fastboot`
...
到 [Releases · woa-lge/msmnilePkg](https://github.com/woa-lge/msmnilePkg/releases) 下载 `lg-mh2lm.img` 或 `lg-mh2lmGPUOC.img`（超频）镜像，放到工具箱的以下位置（记得备份原文件）
```
...
4. 根据提示授权，完成驱动安装
5. 如果看到任何 **App Packages** 的安装报错，忽略即可
6. 安装完成后重启手机即可
